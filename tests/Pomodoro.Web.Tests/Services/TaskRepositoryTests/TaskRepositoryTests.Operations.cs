...
public partial class TaskRepositoryTests
{
    ...
    [Fact]
    public async Task SoftDeleteAsync_WithExistingTask_ReturnsTrue()
    {
        // Arrange
        var taskId = Guid.NewGuid();
        ...
        // Act
        var result = await repository.SoftDeleteAsync(taskId);
    ...
    [Fact]
    public async Task SoftDeleteAsync_WithNonExistingTask_ReturnsFalse()
    {
        // Arrange
        MockIndexedDb
            .Setup(x => x.GetAsync<TaskItem>(It.IsAny<string>(), It.IsAny<string>()))
        ...
        // Act
        var result = await repository.SoftDeleteAsync(Guid.NewGuid());
    ...
    public async Task SoftDeleteAsync_SetsIsDeletedAndDeletedAt()
    {
        // Arrange
        var taskId = Guid.NewGuid();
        var task = CreateSampleTask(id: taskId, isDeleted: false);
        ...
        MockIndexedDb
            .Setup(x => x.PutAsync(It.IsAny<string>(), It.IsAny<TaskItem>()))
            .Callback<string, TaskItem>((_, t) => savedTask = t)
            .ReturnsAsync(true);

        var repository = CreateRepository();

        // Act
        await repository.SoftDeleteAsync(taskId);

        // Assert
        Assert.NotNull(savedTask);
        Assert.True(savedTask.IsDeleted);
        Assert.NotNull(savedTask.DeletedAt);
    }
    ...
    [Fact]
    public async Task HardDeleteAsync_WithExistingId_ReturnsTrue()
    {
        // Arrange
        var taskId = Guid.NewGuid();
        ...
        // Act
        var result = await repository.HardDeleteAsync(taskId);
    ...
    [Fact]
    public async Task HardDeleteAsync_WithNonExistingId_ReturnsFalse()
    {
        // Arrange
        var taskId = Guid.NewGuid();
    ...
    [Fact]
    public async Task HardDeleteAsync_CallsIndexedDbWithCorrectParameters()
    {
        // Arrange
        var taskId = Guid.NewGuid();
        ...
        // Act
        await repository.HardDeleteAsync(taskId);

        // Assert
        MockIndexedDb.Verify(
            x => x.DeleteAsync(Constants.Storage.TasksStore, taskId.ToString()),
            Times.Once);
    }
