...
Phases 0 + 1 + 2 + 4 (read-only UI only). Phase 3 (2-way sync) and Phase 5 (full tests) deferred.
...
5. `TaskItem`: add `GoogleTaskId?`, `GoogleListId?`, `UpdatedAt`, `Notes?`, `DueDate?` (mark Google ids
   `[JsonIgnore]`/strip in `ExportService`). Add `TaskItem.WithUpdates(...)` helper; route the 5 copy
   methods (TaskService + ImportService) through it. `TaskListRef` for tabs; `GoogleTasksSettings` (visible/color) via settings repo.
...
8. Bump `Constants.Storage.DatabaseVersion` 1→2; add `pomo_meta` store (additive) in `indexedDbInterop.js`
   `onupgradeneeded`. Sidecar keyed by GoogleTaskId; joined on read; register sidecar repo in DI.
   For cross-device: include sidecar in `SyncEnvelope.Data` payload + bump `SyncEnvelope.Version` (`CloudSyncService.cs:303/469`) with back-compat read.
